...
`scripts/` 目录用于承载开发辅助流程，避免把复杂逻辑都堆在 `package.json` 的命令行里。

- `scripts/start-debug.ps1`
  - 一键启动本地调试：自动探测 `zotero.exe`、设置 `ZOTERO_PLUGIN_ZOTERO_BIN_PATH` 和 `ZOTERO_PLUGIN_PROFILE_PATH`，并默认将开发 Profile 指向项目根目录下的 `debug/`，最后执行 `npm start`。
- `scripts/gen-env-setup.mjs`
  - 测试前置脚本：读取 `.env`，生成 `test/00.env-setup.test.ts`，把环境变量映射为 Zotero 偏好设置，确保测试可重复执行。
- `scripts/create-provider.mjs`
  - Provider 脚手架：快速创建新的 LLM Provider 模板文件、对应测试文件，并自动在 `.env` 追加占位配置，减少新增厂商适配时的重复工作。
- `scripts/bundle-markmap.mjs`
  - Markmap 资源打包脚本：将 `d3`、`markmap-lib`、`markmap-view` 打包为浏览器可直接使用的 `addon/content/markmap-bundle.js`。
- `scripts/markmap-entry.ts`
  - Markmap 打包入口：负责将依赖挂到 `window` 供页面使用，是 `bundle-markmap.mjs` 的入口文件。
